...
from apps.self_dist.transformer import (
    LMTransformerArgs,
    LMTransformer,
    ...
    with ExitStack() as context_stack:
        ...
        while train_state.step < args.steps:
            ...
            if args.eval is not None and (every_n_steps(
                ...
                - evalの処理を完全に排除した．
                    - lm-evalを利用して評価をするため．
                - eval.py から consolidate_checkpoints をインポートし，評価前にチェックポイントを統合するようにした．
                """
                if not torch.distributed.is_initialized():
                    setup_torch_distributed(DistributedArgs())

                latest_ckpt_dir = (
                    Path(checkpoint.existing_saves[-1])
                    if checkpoint.existing_saves
                    else Path(args.checkpoint.path)
                )

                if (
                    latest_ckpt_dir.exists()
                    and (latest_ckpt_dir / "params.json").exists()
                    and next(latest_ckpt_dir.glob("*.pth"), None) is not None
                ):
                    consolidate_path = latest_ckpt_dir
                else:
                    consolidate_path = latest_ckpt_dir / CONSOLIDATE_FOLDER
                    if not consolidate_path.exists() and get_global_rank() == 0:
                        consolidate_path = consolidate_checkpoints(str(latest_ckpt_dir))

                Path(args.dump_dir).mkdir(parents=True, exist_ok=True)
                dump_config(args, Path(args.dump_dir) / "config.yaml", log_config=False)
                ...
                # from apps.self_dist.eval import (
                #     launch_eval,
                #     EVAL_FOLDER_NAME,
                #     EvalArgs,
                # )
                ...
                # # checkpoint.existing_savesが空の場合の適切な処理
                # if checkpoint.existing_saves:
                #     # 最新のチェックポイントを使用
                #     eval_args.ckpt_dir = str(checkpoint.existing_saves[-1])
                # else:
                #     # チェックポイントがまだ保存されていない場合、現在のジョブのチェックポイントディレクトリを使用
                #     eval_args.ckpt_dir = args.checkpoint.path
                #     logger.warning(
                #         f"No saved checkpoints found, using checkpoint directory for evaluation: {args.checkpoint.path}"
                #     )
